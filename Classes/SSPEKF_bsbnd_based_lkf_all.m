classdef SSPEKF_bsbnd_based_lkf_all
   properties
      x_est_v
      P_est_v
      x_pred_v
   ...
   methods
      function obj = SSPEKF_bsbnd_based_lkf_all()
      ...
      function obj = correct(obj,bss,T,t_idx)
          ...
          for bs_idx = 1:scene.N_bs
            y = mean_z(:,bs_idx) -prior_means(:,bs_idx);
            H_i = eye(2);
            S = H_i*prior_vars(:,:,bs_idx)*(H_i.') + var_z(:,:,bs_idx);
            K = prior_vars(:,:,bs_idx)*(H_i.')*pinv(S);
            
            mean_xy(:,bs_idx) = prior_means(:,bs_idx) + K*y;
            var_xy(:,:,bs_idx) = (eye(2)-K*H_i)*prior_vars(:,:,bs_idx);
          end
