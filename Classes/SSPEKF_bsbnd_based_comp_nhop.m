classdef SSPEKF_bsbnd_based_comp_nhop
   properties
      x_est_v
      P_est_v
      x_pred_v
   ...
   methods
      function obj = SSPEKF_bsbnd_based_comp_nhop()
      ...
      function obj = set_x0(obj,x_0_v)
          scene = Params.get_scene();
          % init vector versions: (same initialization:)
          obj.x_est_v = x_0_v;
      ...
      function obj = correct(obj,bss,T,t_idx)
          ...
          % ---------------------------------------------------------------
          % N-HOP: only for predictions:
          An = eye(scene.N_bs);
          for iter_idx = 1:obj.N_iter
              An = An + (network.A)^iter_idx;
          end
          An(An>0) = 1;
          An = logical(An);
          
          prior_means = zeros(2,scene.N_bs);
          prior_vars = zeros(2,2,scene.N_bs);
          for bs_idx = 1:scene.N_bs
              prior_means(:,bs_idx) = mean(obj.x_pred_v(:,An(bs_idx,:)),2);
              prior_vars(:,:,bs_idx) = mean(obj.P_pred_v(:,:,An(bs_idx,:)),3);
          end
